...
# ── Result Parser ──────────────────────────────────────────────────────────────
def parse_result_to_df(raw: str):
    """
    Try to convert the raw LangChain query result string into a DataFrame.
    Returns a DataFrame on success, or None if parsing fails.
    """
    raw = raw.strip()
    if not raw or raw.startswith("❌") or raw.startswith("⚠️") or raw.startswith("✅"):
        return None
    try:
        # LangChain returns results as a string like: [(val, val), ...] or [(val,), ...]
        parsed = ast.literal_eval(raw)
        if isinstance(parsed, list) and len(parsed) > 0:
            if isinstance(parsed[0], (tuple, list)):
                # Try to extract column names from the SQL
                sql = st.session_state.current_result.get("sql", "")
                # Simple regex to pull SELECT aliases / column names
                ...
                if select_match:
                    ...
                    for p in parts:
                        p = p.strip()
                        # Use alias if present (AS alias or just trailing word)
                        alias_match = re.search(r"(?:AS\s+)?(\w+)\s*$", p, re.IGNORECASE)
                        cols.append(alias_match.group(1) if alias_match else p)
                    if len(cols) != len(parsed[0]):
                        cols = None
                return pd.DataFrame(parsed, columns=cols)
            elif isinstance(parsed[0], dict):
                return pd.DataFrame(parsed)
        # Single scalar result like [(42,)]
        if isinstance(parsed, list) and len(parsed) == 1:
            return pd.DataFrame(parsed, columns=["result"])
    except Exception:
        pass
    return None
...
if st.session_state.current_result:
    ...
    is_info  = output["result"].startswith(("⚠️", "✅"))
    ...
    if show_sql or output["sql"] == "None":
        ...
        with right_col:
            badge = "<span class='badge-error'>Error</span>" if is_error else "<span class='badge-success'>Success</span>"
            st.markdown(f"""
            <div class="result-card-label" style="margin-bottom:0.6rem;">
                Result &nbsp; {badge}
            </div>
            """, unsafe_allow_html=True)
            ...
            if is_error or is_info:
                ...
                df = parse_result_to_df(output["result"])
        ...
        # SQL hidden — result spans full width
        badge = "<span class='badge-error'>Error</span>" if is_error else "<span class='badge-success'>Success</span>"
        st.markdown(f"""
        <div class="result-card-label" style="margin-bottom:0.6rem;">
            Result &nbsp; {badge}
        ...
        if is_error or is_info:
            st.markdown(f'<div class="result-block">{output["result"]}</div>', unsafe_allow_html=True)
        else:
            df = parse_result_to_df(output["result"])
            if df is not None and not df.empty:
